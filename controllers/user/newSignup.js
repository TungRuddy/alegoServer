...
router.get('/:idUser/:email', function (req, res, next) {
    ...
    var sql = "select status from `user` where `id_user`=? and `email`=?";
    mysql.conn.query(sql, [idUser, email], function (error, results, fields) {
        
        if (results[0]) {
            if (results[0].status == '0') {
                var sqlUpdate = "update user set `status` =? where `id_user`=? and `email`=?";
                mysql.conn.query(sqlUpdate, ['1', idUser, email], function (error, data, fields) {
                    if (error) throw error;
                    return res.send(`
                    <!DOCTYPE html>
<html lang="en">
<head>
  <title>Agrilego Active Account</title>
  <meta charset="utf-8">
...
<div class="container">
	<h2>Tài khoản của bạn được kích hoạt thành công ! Cảm ơn.</h2>
